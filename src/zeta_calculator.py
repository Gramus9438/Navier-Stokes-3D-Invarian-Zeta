...
class NavierStokes3D:
    ...
    def irfft3(self, a_hat: Array) -> Array:
        return np.fft.ifftn(a_hat).real
    ...
    def nonlinear_hat(self, u_hat: Array) -> Array:
        # IMPORTANT: inverse FFT per component (no transform over component axis)
        u0 = self.irfft3(u_hat[0])
        u1 = self.irfft3(u_hat[1])
        u2 = self.irfft3(u_hat[2])

        # gradients per component (physical space)
        ...
        # advective term components in physical space
        adv_x = u0 * dux_dx + u1 * dux_dy + u2 * dux_dz
        adv_y = u0 * duy_dx + u1 * duy_dy + u2 * duy_dz
        adv_z = u0 * duz_dx + u1 * duz_dy + u2 * duz_dz
...
# 7) Truncated jet (mean flow + small-scale perturbations)
...
# 9) Small scales superposition (already above as helper) — keep mapping
# (ic_small_scale_superposition)
